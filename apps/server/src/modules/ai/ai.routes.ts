...
export const aiRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post('/parse-recipe', async (request, reply) => {
    const parseResult = textAnalysisSchema.safeParse(request.body);
    if (!parseResult.success) {
      return reply.status(400).send({ error: parseResult.error.flatten() });
    }
    ...
    try {
      const result = await geminiService.parseRecipeText(parseResult.data.prompt);
      return reply.send(result);
    } catch (err: any) {
      request.log.error(err);
      return reply.status(500).send({
        error: 'Failed to generate recipe with Gemini AI',
        message: err.message,
      });
    }
  });
