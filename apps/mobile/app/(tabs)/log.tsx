...
export default function LogScreen() {
  ...
  // Debounced search: an empty query returns the top 20 foods. Falls back to
  // the local SQLite cache when the server can't be reached.
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      {/* Search Input */}
      <View className="flex-row items-center bg-slate-900/80 border border-slate-800 rounded-2xl px-4 py-3 mb-4">
      ...
      {!searchLoading &&
        foods.map((food) => (
          <View
            key={food.id}
            className="bg-slate-900/60 border border-slate-800/80 rounded-2xl mb-2.5 overflow-hidden"
          >
            <TouchableOpacity
